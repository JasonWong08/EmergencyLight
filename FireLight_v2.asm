...
CHARGE_BAT_CTRL:
	...
	LDA	BAT_STATE		
	BA2	CHARGE_BAT_ENABLE	;如果电池电压过低，则开始对电池充电
...
KEY_CHECK:
	ADI	F_496MS_1S,	0001B	;检查496MS标志位
	BA0	KEY_PROCESS_END		;未到496ms，暂时不做按键扫描

	ANDIM	F_496MS_1S,	1110B	;清496MS标志位
...
MORE_7S:
	...
	JMP	RELEASED_OVER	
LESS_3S:
	LDI	PRESS_DURATION,	0000B	;清除"小于3秒"状态
	JMP	RELEASED_OVER
LESS_5S:
...
KEY_PRESSED:
	...
	;ORIM	PRESS_DURATION,	0001B	;按键持续时长小于3S, 置"小于3秒"状态
	;JMP	PRESSED_DURATION_ADD_1
	
;MORE_3S:
	;ANDIM	PRESS_DURATION,	1110B	;按键持续时长大于3S, 清除"小于3秒"状态
...
KEY_PRESSED_CHK:
	SBI	BTN_PRESS_CNT,	06H	;
	BNC	KPC_LESS_3S		;按键持续时长小于3S, 6.04 * 496ms = 3s
	SBI	BTN_PRESS_CNT,	0AH	;
	BNC	KPC_LESS_5S		;按键持续时长小于5S, 10.08 * 496ms = 5s
	SBI	BTN_PRESS_CNT,	0EH	;
	BNC	KPC_LESS_7S		;按键持续时长小于7S, 14.11 * 496ms = 7s
...
MANUAL_CHK:
	...
	LDI	GREEN_FLASH,	00H	;如果当前没有自检，则让绿灯停止闪烁
...
SCS_LESS_3S:
	ADI	BTN_PRE_STA,	01H
	BA0	BTN_PRE_PRESSED		;如果当前按键处于按下状态，则跳转

	ANDIM	SELF_STATE,	0110B	;清模拟应急标志位

	ADI	SELF_STATE,	1000B	;
	BA3	SCS_CLEAR_DURATION	;如果当前不是手动月检或手动年检，则跳转
	
	BA1	SCS_SIMU_MONTH		;如果当前为手动月检，则跳转
	BA2	SCS_SIMU_YEAR		;如果当前为手动年检，则跳转	
	JMP	SCS_CLEAR_DURATION

SCS_SIMU_MONTH:
	ANDIM	SELF_STATE,	0101B	;退出手动月检
	JMP	SCS_CLEAR_DURATION

SCS_SIMU_YEAR:
	ANDIM	SELF_STATE,	0011B	;退出手动年检
	JMP	SCS_CLEAR_DURATION
...
SCS_LESS_5S:
	...
	ADI	BTN_PRE_STA,	01H
	BA0	SCS_CLEAR_DURATION	;如果当前按键为按下状态，则跳转
...
SCS_CLEAR_DURATION:
	ANDIM	PRESS_DURATION,	1000B	;清按键时长标志低三位
...
SCP_MONTH_1S:
	...
	BA1	SELF_CHK_PROCESS_END	;还未到1秒，则跳转

	ANDIM	F_496MS_1S,	1101B	;清供应急计时用的1秒标志位
	...
	JMP	SELF_CHK_PROCESS_END	;如果电池还未耗尽，则跳转
...
SCP_ARRIVE_120S:
	...
	ANDIM	ALREADY_ENTER,	1011B	;清已经进入手动月检标志位
